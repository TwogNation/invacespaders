...
const scriptsInEvents = {
	...
	async Gamesheet_Event10_Act2(runtime, localVars)
	{
	...
	async Gamesheet_Event15_Act2(runtime, localVars)
	{
		runtime.globalVars.webSocket.onMessage = (event) => {
			if (event.data.startsWith('s:')){
				const score_ = event.data.split(':')[1];
	...
	async Gamesheet_Event28_Act12(runtime, localVars)
	{
	...
	async Gamesheet_Event48_Act4(runtime, localVars)
	{
		console.log(10)
	},

	async Gamesheet_Event48_Act5(runtime, localVars)
	{
		runtime.globalVars.webSocket.send("s:10")
		
	},

	async Homesheet_Event13_Act1(runtime, localVars)
	{
		...
		console.log("clicked")
					...
					const button = runtime.objects.txtPlay.getFirstInstance();
					button.destroy(); 
